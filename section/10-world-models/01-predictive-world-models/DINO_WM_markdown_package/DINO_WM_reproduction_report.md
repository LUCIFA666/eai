...
![结果摘要](assets/result_summary.png)

## 2. 复现流程总览

![复现流程](assets/pipeline.png)
...
**预期结果：** `$REPORT_DIR/assets` 下生成 `extended_metrics.csv`、`extended_results_summary.json` 和 `extended_metrics_curve.png`；若日志缺少 `final_eval`，命令会明确失败。
...
"${CONDA_ENV}/bin/python" "${REPORT_DIR}/scripts/analyze_extended_results.py" --pusht-dir "${PUSHT_RUN_DIR}" --wall-dir "${WALL_RUN_DIR}" --output-dir "${REPORT_DIR}/assets"
```
...
![指标曲线](assets/metrics_curve.png)

### 5.2 规划过程拼图

前 5 次 MPC 迭代未成功，第 6 次成功。最终输出视频为 `output_final_0_success.mp4`。

![规划过程拼图](assets/plan_montage.png)

### 5.3 磁盘占用

![磁盘占用](assets/disk_footprint.png)
...
![PushT 与 Wall 指标曲线](assets/extended_metrics_curve.png)

PushT 最终规划截图：

![PushT 最终规划截图](assets/pusht_extended_final.png)

Wall 最终规划截图：

![Wall 最终规划截图](assets/wall_extended_final.png)
...
| `assets/extended_metrics.csv` | PushT/Wall 每个 MPC step 的成功率和距离指标 |
| `assets/extended_results_summary.json` | PushT/Wall 最终成功数、成功率和 Wilson 95% 区间 |
| `assets/extended_metrics_curve.png` | PushT/Wall 指标曲线 |
| `assets/pusht_extended_final.png` | PushT 最终规划截图 |
| `assets/wall_extended_final.png` | Wall 最终规划截图 |
