...
![Breakout 原始帧与 tokenizer 重建帧对照](../../../repro_assets/tokenizer_reconstruction_sequence.png)

如果这里没有显示图片，请确认你打开的是整个 `iris_repro` 目录里的 Markdown，
而不是只拷贝了单个 `.md` 文件。图片文件位于：

```text
repro_assets/tokenizer_reconstruction_sequence.png
```
...
![Breakout evaluation test episode sequence](../../../repro_assets/evaluation_test_episode_sequence.png)
...
![Breakout evaluation test episode gif](../../../repro_assets/evaluation_test_episode.gif)

评价脚本还保存了 world model imagination 轨迹。因为本指南中的冒烟验证
把 `evaluation.actor_critic.horizon=2`，所以这里的想象轨迹只有 2 帧：

![Breakout short imagination rollout](../../../repro_assets/evaluation_imagination_short_rollout.png)
