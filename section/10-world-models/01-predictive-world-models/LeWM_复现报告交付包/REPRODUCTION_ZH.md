...
![LeWM 复现流程总览](assets/reproduction-pipeline.png)
...
![复现检查结果](assets/reproduction-checks.svg)

### 可追溯的真实验证证据

下图不是示意图，而是本目录重新执行 `repro_smoke.py` 后的真实输出摘录；完整原始输出见 [smoke_test.log](artifacts/verification/smoke_test.log)。

![真实 smoke 测试终端输出](assets/verification-smoke-terminal.svg)

训练验证使用同一数据 schema 的合成 HDF5，仅证明数据加载、前向、反向和保存流程可运行；原始终端输出见 [synthetic_train.log](artifacts/verification/synthetic_train.log)，指标见 [synthetic_train_metrics.csv](artifacts/verification/synthetic_train_metrics.csv) 与 [synthetic_train_summary.json](artifacts/verification/synthetic_train_summary.json)。

![真实合成 HDF5 训练验证](assets/verification-training-terminal.svg)

### 真实 Push-T 控制结果（50 episode）

这是本目录实际运行官方完整评测配置所得，而不是 README 的示意 GIF：`num_eval=50`、`eval_budget=50`、CEM `num_samples=300`、`n_steps=30`，随机种子 42。结果为 **48 / 50 成功，success_rate = 96.0%**，评测耗时 363.62 秒。完整机器可读指标见 [pusht_real_50_results.txt](artifacts/real_result/pusht_real_50_results.txt)，完整运行日志见 [pusht_real_50.log](artifacts/real_result/pusht_real_50.log)。

<video controls muted loop playsinline width="480" poster="artifacts/real_result/pusht_real_50_first_frame.png">
  <source src="artifacts/real_result/pusht_real_50.mp4" type="video/mp4">
  当前 Markdown 预览器不支持内嵌视频；请打开 <a href="artifacts/real_result/pusht_real_50.mp4">pusht_real_50.mp4</a>。
</video>

**真实视频文件：** [pusht_real_50.mp4](artifacts/real_result/pusht_real_50.mp4)。上面的播放器与此文件均为本次 50 episode 评测的第 0 个环境轨迹。

![真实 50 episode 评测中第 0 个环境的控制轨迹](artifacts/real_result/pusht_real_50.gif)

![真实 50 episode 评测视频首帧](artifacts/real_result/pusht_real_50_first_frame.png)
...
<p align="center">
  <img src="assets/lewm.gif" width="88%" alt="LeWorldModel 官方规划演示 GIF">
</p>
...
本复现目录已经提供 [repro_smoke.py](repro_smoke.py)：
...
| 真实评测耗时 | 363.62 秒 |
| 真实视频 / GIF / 原始指标 | 已保存于 `artifacts/real_result/` |
